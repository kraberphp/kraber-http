...
{
    /** @var array<string, mixed> URI components.*/
    ...
    {
        $characters = preg_quote(self::UNRESERVED_CHARACTERS . self::RESERVED_CHARACTERS, "/");

        return preg_replace_callback(
            "/(?:[^" . $characters . "%:@\/]+|%(?![a-fA-F0-9]{2}))/",
    ...
    public function withPort($port = null): static
    {
        if ((!is_int($port) && !empty($port)) || ($port < 0 || $port > 0xffff)) {
            throw new InvalidArgumentException("Argument provided must be an integer between 1 and 65535 or null.");
        }
